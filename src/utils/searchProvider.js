...
exports.D2SearchProvider = void 0;
const vscode = __importStar(require("vscode"));
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
class D2SearchProvider {
    async searchAcrossDatasets(searchTerm, datasets) {
        const results = [];
    ...
    async searchInDataset(dataset, searchPattern) {
        const results = [];
        try {
    ...
    async searchInFile(filePath, fileName, datasetName, searchPattern) {
        const result = {
            dataset: datasetName,
            file: fileName,
            filePath: filePath,
    ...
    showResults(results) {
        ...
        vscode.window.showInformationMessage(`Found ${totalMatches} matches in ${results.length} files`, 'Show Details').then(selection => {
            if (selection === 'Show Details') {
                this.openSearchResultsDocument(results);
            }
        });
    }
    async openSearchResultsDocument(results) {
    ...
    async searchInCurrentFile() {
        ...
        const result = await this.searchInFile(activeEditor.document.fileName, fileName, 'Current File', searchPattern);
...
exports.D2SearchProvider = D2SearchProvider;
